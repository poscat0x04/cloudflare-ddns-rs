    ...
    for addr in addrs {
        ...
        match addr.family() {
            Some(Inet) => {
                let ip = addr
                    .as_sockaddr_in()
                    .expect("Impossible, failed to cast to sockaddr_in")
                    .ip();
            ...
            Some(Inet6) => {
                let ip = addr
                    .as_sockaddr_in6()
                    .expect("Impossible, failed to cast to sockaddr_in6")
                    .ip();
                v6.push(ip)
            }
...
pub fn get_interested_addrs(if_name: &str) -> Result<Addrs> {
    ...
    let filtered_v4 = all_v4
        .into_iter()
        .filter(|addr| !addr.is_link_local() && !addr.is_loopback());
    let filtered_v6 = all_v6.into_iter().filter(|addr| {
...
mod test {
    ...
    use crate::interface::{get_addrs, get_interested_addrs, Addrs};
    ...
    fn test_get_interested_addrs() -> Result<()> {
        let Addrs { v4_addrs, v6_addrs } = get_interested_addrs("ens34")?;
